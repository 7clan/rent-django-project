...
from .forms import RenterForm, FloorForm, ApartmentForm, YearlyRentForm
...
class RenterDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        payment_statuses = self.object.payment_status_by_month()
        normalized_payments = {}
        for item in payment_statuses:
            year, month = item["month"].split("-")
            normalized_payments[f"{year}-{int(month):02}"] = item
        current_year = self.object.start_date.year
        if payment_statuses:
            current_year = int(payment_statuses[0]["month"].split("-")[0])
        ...
        for i, month in enumerate(months):
            ...
            for year in years:
                ym = f"{year}-{month}"
                month_date = date(year, int(month), 1)
                row["statuses"].append(
                    normalized_payments.get(
                        ym,
                        self.object.payment_status_for_month(month_date),
                    )
                )
            table.append(row)
        context["years"] = years
        context["monthly_rents_by_year"] = [
            {"year": year, "amount": self.object.monthly_rent_for_year(year)}
            for year in years
        ]
...
def floor_page(request):
    ...
    yearly_rent_form = YearlyRentForm()
    ...
    if request.method == "POST":
        ...
        if action == "add_floor":
            ...
            return JsonResponse({"error": floor_form.errors}, status=400)
        elif action == "add_apartment":
            apartment_form = ApartmentForm(data)
            if apartment_form.is_valid():
                apartment_form.save()
                return JsonResponse({"success": True})
            return JsonResponse({"error": apartment_form.errors}, status=400)
        elif action == "add_yearly_rent":
            yearly_rent_form = YearlyRentForm(data)
            if yearly_rent_form.is_valid():
                yearly_rent_form.save()
                return JsonResponse({"success": True})
            return JsonResponse({"error": yearly_rent_form.errors}, status=400)
        elif action == "add_renter":
            renter_form = RenterForm(data)
            if renter_form.is_valid():
                renter_form.save()
                return JsonResponse({"success": True})
            return JsonResponse({"error": renter_form.errors}, status=400)
    ...
    context = {
        ...
        "yearly_rent_form": yearly_rent_form,
...
class FloorListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["yearly_rent_form"] = YearlyRentForm()
...
@csrf_exempt
@jwt_required
def leave_apartment(request, renter_id):
    ...
    renter.move_out_date = move_out_date
    renter.save()
    return JsonResponse({"success": True})
