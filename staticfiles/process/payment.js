        ...
        try {
            ...
            if (res.ok) {
                ...
                if (payment_type === 'monthly') {
                    ...
                    // mark 12 months from January of the covered year
...
if (form) {
    form.addEventListener("submit", async (e) => {
        ...
        const amount = form.amount.value;
        const year_month_covered = form.year_month_covered.value;

        if (!amount || !year_month_covered) {
        ...
        try {
            const res = await fetch(form.action, {
                ...
                body: JSON.stringify({
                    amount: amount,
                    year_month_covered: year_month_covered
                })
            });
            ...
            if (res.ok) {
                ...
                // Update the payment matrix table
                const [year, month] = year_month_covered.split("-");
                const cells = document.querySelectorAll("table.payment-matrix tbody td");
                
                // Find the correct row (month) and column (year)
                const monthNames = ["January", "February", "March", "April", "May", "June",
                                  "July", "August", "September", "October", "November", "December"];
                const monthIndex = parseInt(month) - 1;
                const monthName = monthNames[monthIndex];
                
                // Find row with this month
                const rows = document.querySelectorAll("table.payment-matrix tbody tr");
                for (let row of rows) {
                    const monthCell = row.querySelector("td");
                    if (monthCell && monthCell.textContent === monthName) {
                        ...
                        const yearIndex = headers.findIndex(h => h.textContent.trim() === year);
